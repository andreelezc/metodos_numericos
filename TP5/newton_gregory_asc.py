...
def formula(vector_x, vector_y, n):
    suma = vector_y[0]
    u = (valor - vector_x[0]) / (vector_x[1] - vector_x[0])
    for i in range(1, n):
        suma = suma + (calcular_u(u, i) * tabla_dif[0][i]) / math.factorial(i)
    return suma


# Resultados
cprint('{: ^80}'.format(" NEWTON GREGORY "), 'blue', attrs=['bold'], end="\n")
cprint('{: ^80}'.format(" Ascendente "), 'red', attrs=['bold'], end="\n")
...
# Valor a interpolar
valor = float(input('Valor a interpolar: '))  # 17
